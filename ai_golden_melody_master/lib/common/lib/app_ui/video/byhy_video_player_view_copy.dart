...
class VideoPlayerWidgetCopyState extends State<VideoPlayerWidgetCopy>
  ...
  void initState() {
    ...
    // 初始化远程视频控制器
    if (widget.url.startsWith("http")) {
      _cachedVideoPlayerPlusController = CachedVideoPlayerPlusController
          .networkUrl(
        Uri.parse(widget.url),
      )..initialize().then((_) async {
          _cachedVideoPlayerPlusController?.setVolume(widget.mute ? 0.0 : 1);
          // 使用initialPosition或offset设置初始播放进度
          Duration initialSeek =
              widget.initialPosition ?? Duration(seconds: widget.offset ?? 0);
          await _cachedVideoPlayerPlusController!.seekTo(initialSeek);
        ...
        ..initialize().then((_) {
          _localController!.setLooping(true).then((_) async {
            if (mounted) {
              setState(() {});
              // 使用initialPosition或offset设置初始播放进度
              Duration initialSeek = widget.initialPosition ??
                  Duration(seconds: widget.offset ?? 0);
              await _localController!.seekTo(initialSeek);
              if (widget.maxDuration != null) {
