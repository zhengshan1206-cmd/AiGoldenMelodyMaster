...
class DioUtils {
  ...
  Future<void> _updateUserAgent() async {
    late final String? userAgents;

    if (Platform.isAndroid) {
      userAgents = await UserAgentUtil.getUserAgent();
    }

    if (Platform.isIOS) {
      /// userAgents = await UserAgentUtil.getUserAgent();
      userAgents = FkUserAgent.webViewUserAgent;
    }

    if (userAgents == null) {
      _dio.options.headers[ConstKeys.kUserAgent] = ConstKeys.userAgentData;
    }
    _dio.options.headers[ConstKeys.kUserAgent] = userAgents;
    Get.log("userAgent=====>  $userAgents");
  }
