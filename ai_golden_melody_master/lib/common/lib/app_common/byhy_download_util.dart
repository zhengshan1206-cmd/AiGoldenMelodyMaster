...
class ByDownloadUtil {
  ...
  static Future<void> downloadAudio(
    ...
    try {
      ...
      EasyLoading.dismiss();
      if (!fileRenameExists) {
        EasyLoading.showToast("音频下载失败");
        return;
      }
      onSuccess?.call(filePath);
    } catch (e) {
      EasyLoading.dismiss();
      EasyLoading.showToast("文件下载失败,请稍后重试");
      onFailed?.call();
    }
  }
  ...
  static Future<AssetEntity?> saveVideoToAlbum(
    ...
    if (!exists) {
      EasyLoading.dismiss();
      if (isToast) {
        EasyLoading.showToast("保存视频到相册失败");
      }
      return null;
    }
    ...
    EasyLoading.dismiss();
    if (isToast) {
      EasyLoading.showToast("视频已保存到相册中");
    }
    return asset;
  }
  ...
  static Future<AssetEntity?> saveVideo2Album(
    ...
    fileName ??= filePath.split(Platform.pathSeparator).last;
    final asset = await ImageGallerySaver.saveFile(filePath);
    EasyLoading.dismiss();
    File(filePath).delete();
    if (asset != null) {
      if (isToast) {
        EasyLoading.showToast("保存到相册成功");
      }
    } else {
      if (isToast) {
  ...
  static Future<AssetEntity?> downloadImage(
    ...
    if (await Permission.storage.request().isGranted) {
      try {
        ...
        EasyLoading.showToast("封面下载成功~",maskType:EasyLoadingMaskType.none );

        return asset;
      } catch (e) {
        EasyLoading.dismiss();
        EasyLoading.showToast("文件下载失败,请稍后重试",maskType:EasyLoadingMaskType.none);
        return null;
      }
    } else {
      EasyLoading.dismiss();
      EasyLoading.showToast("请现在设置中打开文件访问权限",maskType:EasyLoadingMaskType.none);
      return null;
    }
  }
  ...
  static Future<void> saveNetwrokImage(
    ...
    if (result['isSuccess']) {
      EasyLoading.showToast("封面下载成功,请在相册中查看~",maskType:EasyLoadingMaskType.none );
    } else {
      EasyLoading.showToast("封面下载失败，请重试~",maskType:EasyLoadingMaskType.none );
    }
  }
}
